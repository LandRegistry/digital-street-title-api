...
Revision ID: 0f5345ccf2a5
Revises: 
Create Date: 2018-12-20 15:43:17.836448
...
def upgrade():
    ...
    sa.Column('charge_date', sa.DateTime(), server_default=sa.text('now()'), nullable=False),
    ...
    sa.Column('restriction_date', sa.DateTime(), server_default=sa.text('now()'), nullable=False),
    sa.Column('title_number', sa.String(), nullable=True),
    sa.Column('charge_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['charge_id'], ['charge.charge_id'], onupdate='CASCADE', ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['title_number'], ['title.title_number'], ),
    ...
    context.execute("GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE \"address\", \"conveyancer\", \"owner\", \
                    \"title\", \"charge\", \"restriction\" TO " + current_app.config.get('APP_SQL_USERNAME'))
    context.execute("GRANT USAGE ON SEQUENCE \"address_address_id_seq\", \
                    \"charge_charge_id_seq\", \
                    \"conveyancer_conveyancer_id_seq\", \
                    \"restriction_restriction_id_seq\" TO " + current_app.config.get('APP_SQL_USERNAME'))
